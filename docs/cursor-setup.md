...
Claude Code auto-loads the `using-neo` meta-skill through its `SessionStart` hook. Cursor has no equivalent, so add the rule below as an **always-applied** rule (e.g. `.cursor/rules/00-load-neo.md` with `alwaysApply: true`) so neo drives the flow instead of the agent improvising:
