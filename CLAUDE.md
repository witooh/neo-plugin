...
- Shared references live in top-level `references/` (the source of truth); `scripts/bundle-references.sh` fans a copy into every skill whose SKILL.md cites one, so each skill dir is self-contained for installers that copy skill dirs as-is (Kiro, pi). Never hand-edit the per-skill copies — they are regenerated on every upstream sync. Skill-specific references live inside their own skill
...
3. **Publish a GitHub release** — this is the changelog home (there is no `RELEASE.md`). Once the tag is on `origin`, create a release against it with structured notes you write from the diff: a `### <headline>` line, then **Added** / **Changed** / **Removed** / **Notes** sections (match the shape of prior releases). `gh release create v<version> --title "v<version>" --notes-file <tmp.md> --latest` (the release **title is the version only** — the `<headline>` goes in the notes body, not the title) — the newest release gets `--latest`; backfilling an older one uses `--latest=false`. Check prior format first with `gh release list` / `gh release view v<x.y.z>`.
...
- Never: Duplicate content between skills — reference other skills instead (exception: the per-skill copies of shared references generated by `scripts/bundle-references.sh`)
