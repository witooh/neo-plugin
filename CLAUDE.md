...
Skills currently bundled: `neo`, `brainstorm`, `improve`, `api-doc`, `openapi-doc`, `open-collection`, `confluence-api-doc`, `gitlab`, `commit`, `atlassian`. The README's table is the authoritative list of triggers. (The **API-doc family is dual-source**: **Go is the single source of truth**, read by two sibling generators that each verify against Go (so they stay mutually consistent — consistency is transitive, no md↔spec cross-check needed): `api-doc` → Markdown in `docs/api/`, and `openapi-doc` → an OpenAPI 3.2 split spec in `docs/openapi/` (root `openapi.yaml` + `paths/` + `components/schemas/`; business logic in `x-business-logic`, per-sentinel errors in `x-error-catalog`). The two downstream skills each *derive* from **either** source — auto-preferring the spec when both exist, with an override — `open-collection` (a runnable Bruno OpenCollection; reads the md, or imports the spec via `bru import openapi`) and `confluence-api-doc` (Confluence pages; reads the md, or reconstructs pages from the spec incl. the `x-*` extensions). Each of the four carries its own **three-layer verify** (L1 script · L2 fresh-eyes · L3 completeness sweep). History: consolidated into one `api-doc` gen+publish skill in 0.8.0, re-split md-hub in 0.15.0, then `openapi-doc` added + downstream made dual-source in 0.18.0; the `bruno` hand-authoring skill was removed in 0.9.0; `atlassian` — the `acli` Jira/Confluence reference + direct-ops skill, and the acli source `neo`'s BA already points to (`skills/neo/references/shared/jira-ref.md §7`) — was added in 0.10.0 as a lean "thin shell over `acli --help`" rework of the user-global skill. NOTE: `openapi-doc`'s `references/go-scan-patterns.md` is a deliberate **copy** of `api-doc`'s — the Go-reading rules are shared; edit both when either changes.)
