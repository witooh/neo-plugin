...
- `neo` references (`skills/neo/references/`) are **read by the specialist sub-agent** (point-to-read): `loop-over-meta-skill.md` (how each iteration hands off to `using-agent-skills`), `exit-condition.md` (BA's done template), `state-schema.md` (STATE.md shape), `human-gate.md` (commit/PR escalation). Roles: `roles/business-analyst.md` (exit-condition framer) + `roles/librarian.md` (memory primitive). Changing a role's contract here changes specialist behavior.
