...
- **Knowledge base** (`shared/knowledge-base.md` + `roles/librarian.md`). `docs/knowledge/` is **markdown** (registered exception — `html-output.md §8`); the Librarian is its **sole writer**; downstream reads it **context-only** (AC stays binding — `preamble.md §5`). Conflicts are **user-decided**, applied in place by the Librarian (KB) + BA (AC) and logged as a `VERSION.md` changelog entry (`## v<N> — <date>` section, **Sources**/**Change** bullets) — **no `conflicts.md`**, no inline markers. **No catch-all `## Notes`** in any KB artifact. KB *content* may be non-English (the 0-Thai rule binds skill files only). The Ingest-first guard is ID-less (see the gate bullet).
