...
- **Verification is independent-verify, not ceremony.** Doc-adversarial loops back upstream **1 round on semantic/judgment defects** then escalates; **measurable defects (count/grep/CS1) loop until evidence-green, ~3 rounds no-progress → escalate** (same stall wording as the Dev Loop). L2 fresh-eyes reuses the downstream role in **verify-only mode** when a writer runs isolated (folded into CP-final). **Still no budget/max-iteration prose** — "evidence-green OR ~N rounds" only; never delete the independent verify itself. **Apparent contradictions are reconciled against source, not escalated raw:** a specialist reconciles a perceived AC contradiction against the KB/design end-to-end flow before declaring it (`preamble.md §5`); the orchestrator opens the cited source itself before relaying a contradiction to the user (`SKILL.md § Subagent Status`). Both are **ID-less disciplines** (like §1 Never-Guess / the routing guards) — they don't move the 23/156 gate count.
