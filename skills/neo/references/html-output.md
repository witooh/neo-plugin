...
- **`docs/tasks/<card-id>/plan.md`** — the per-card **task-tracking / resume index** (`shared/task-tracking.md`): card-keyed `Build` progress + the AC task table the Orchestrator reads to resume. **Markdown**, never HTML; out of scope of `lint.py` / `docverify.py`; lives under `docs/tasks/` (not `docs/design/`). Written only by BA (`roles/business-analyst.md` § Card Task-File). Card-keyed work only.
