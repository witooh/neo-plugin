...
**Apparent contradiction = reconcile before you report.** If two requirements *seem to conflict*, or an AC reads as ambiguous about **ordering / sequence / a precondition** ("does compare run before or after the write?"), treat it as a **verify-trigger, not a finding**: before you declare a contradiction *or* resolve it by picking a reading, **reconcile against the full source — first the KB's end-to-end flow / sequence (and any design sequence doc)**, where the disambiguator (step order, an "after-X" state) usually lives — not the atomic clause you started from. Resolve-by-source first; **only if the flow genuinely does not settle it** → surface it (Open Question / loop back to BA), never silently pick a reading (that is the Never-Guess of §1). This disambiguates *how to read* the AC; it does **not** let the KB add or override a requirement — the binding rule above stands.

## 6. Report = pointers, not payload
Your closing report is for the orchestrator to route on and the user to read — it is **not** where artifacts live. Do **not** paste into it: full source or test code, raw build/test logs, or raw command output (grep / scanner / type-check dumps) / file contents. All of that is already on disk; the orchestrator and every downstream specialist read it from disk (point-to-read), so relayed copies are never read — they only inflate context. Report **pointers + outcome** instead: the path(s) you wrote or changed, `file:line` for each change or finding, what changed and why, and a verdict / count / failures-list. **Keep** the structured summaries the orchestrator actually consumes (the AC list, the traceability map, the findings table, the test summary) — trim only the duplicated payload behind them. A report that restates a file the reader can open is dead weight, re-sent on every later turn.
