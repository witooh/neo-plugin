...
- **Create (at Spec):** one row per AC — `Readiness` mirrored from the AC `Status`, `Build = pending`, `Depends-on` from each AC's `Blocker:` id. The skeleton needs nothing from later phases; roll-up counts must match the rows.
- **Refresh (after Design):** read the Architect's `traceability.html` (per-AC design-element mapping) → fill sub-task checklists for **big ACs only** (two or more new/independent surfaces) + the **Shared-prerequisites** table (a prereq that unblocks two or more ACs shows as ACs sharing a dependency). Derive from the existing traceability — no Architect change needed.
...
**Spec-only card work** (e.g. "create AC" with no Design phase): write the skeleton and note `sub-tasks/shared-prerequisites pending Design` — a valid state, not an error (Build cannot run in a Spec-only pass); the post-Design refresh fills them later. **Never** write a progress value into the AC `Status` field, and never add a `Status` column to the task-file — readiness (`Ready`/`Blocked`) and progress (`Build`) are separate axes (`../shared/ac-status.md` §6, `../shared/task-tracking.md`).
