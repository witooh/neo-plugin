...
2. **Requirement coverage map (vs SOURCE ARTIFACTS, not just typed text)** — build coverage against **every source item** (read from its `docs/knowledge/` digest — Source-Artifact Fetch above; the KB is the ingested form of what the orchestrator passed in `## Source Artifacts`): (i) typed requirements; (ii) **image/mockup** (read the local path with the Read tool — it renders images — enumerate each field/rule/state → an AC); (iii) **JIRA card content** (fetch via `acli`, see Source-Artifact Fetch above — map each acceptance bullet → an AC); (iv) **requirements added mid-task**. Each source item → the AC-IDs that realize it; + every AC that adds behavior **no source item states** (an inference you thought obvious). A gap an artifact *does* resolve is a coverage miss you fix; only **genuine residual ambiguity no artifact resolves** becomes an Open Question (BA1). Lets the user catch a misread you're unaware of.
