...
**Doc adversarial inline:** Design verifies Spec (AR7), TestSpec verifies Design (Q7) **before** producing work. Blocker → loop back to upstream 1 round → still failing, escalate. BA closes the loop on test cases via the "modify Test Cases" row (BA review). **Dev Loop:** Build→Verify auto-loops until E2E passes + CR/Security have no Blocker/Critical, ~3 rounds then escalate. Cut the budget/max-iter ceremony — keep the independent verify.
...
- **Review (read-only, no auto-fix):**
  1. (8b) resolve the usecase from `docs/design/INDEX.md`, confirm `acceptance-criteria.html` has `JIRA Ref:` = card, keep the paths (`acceptance-criteria.html`, `test-cases.html`, `traceability.html`). card → none / several usecases / unclear → ask the user. Local docs only (don't fetch live JIRA)
