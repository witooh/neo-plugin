...
Task arrives
    │
    ├── Have an external source to capture first? → markitdown  (/ingest)
...
```
0.  markitdown (/ingest)        → Capture external context into the knowledge base (optional)
...
|-------|-------|-----------------|
| Ingest | markitdown | Curate an external source into the knowledge base with provenance |
