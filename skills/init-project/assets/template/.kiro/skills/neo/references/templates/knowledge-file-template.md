...
**Content spec for a knowledge digest and the source manifest.** Read with `~/.kiro/skills/neo/references/shared/knowledge-base.md` (definitions + gates). Written **only by the Librarian** (sole writer of `docs/knowledge/`). Read by the Librarian (refresh) and, **context-only**, by downstream roles (Architect / QA / Developer — `~/.kiro/skills/neo/references/shared/preamble.md §5`).

- **Format: Markdown, never HTML** — a registered exception (`~/.kiro/skills/neo/references/html-output.md §8`); do **not** run `lint.py` / `docverify.py` on it.
- **Topic-named**, kebab-case, timeless (`account-eligibility.md` — not `gi-52.md`, not a date/ticket). One file = one topic; it may aggregate several sources (`~/.kiro/skills/neo/references/shared/knowledge-base.md §5`).
...
- **Current correct state only** — no changelog, no conflict markers, no superseded values (`VERSION.md` is the change trail; `~/.kiro/skills/neo/references/shared/knowledge-base.md §5,§8`).
- **`## References`** lists transitive sources this topic cites (record all; `[needed]` ones are ingested on-need — `~/.kiro/skills/neo/references/shared/knowledge-base.md §9`). Attachments are references too.
- **No `## Notes`** — every fact has a topic line, an inline tag, or an INDEX entry; nothing else is recorded (`~/.kiro/skills/neo/references/shared/knowledge-base.md §10`).
...
## v1.2 — 2026-06-10
- **Sources:** GI-52, confluence:NEOSVC
...
## v1.0 — 2026-06-10
- **Sources:** GI-52, confluence:NEOSVC, image:neobank-flow
...
### confluence:NEOSVC (confluence)
- **Locator:** https://.../wiki/.../NEOSVC
- **Hash:** d4e5f6
...
- **Locator** (Sources) — portable per `~/.kiro/skills/neo/references/shared/knowledge-base.md §4`: URL > parent-URL + filename > bare filename; **never a local path**.
...
- **Language:** a digest mirrors its source's language and **may be non-English**; the language-neutral rule binds skill files only (`~/.kiro/skills/neo/references/shared/knowledge-base.md §5`). A **contract clause** (any clause constraining observable behavior) is quoted **verbatim in the source's original wording** — never paraphrased, never translated, even when the surrounding digest prose is in another language (KB4). *(Placeholders above are English because this template is a skill file.)*
- **Gates:** the Librarian runs KB1 (portable pointer + verify-once), KB2 (INDEX + VERSION integrity), KB3 (staleness → bump VERSION), **KB4** (atomic-clause fidelity self-check; contract clauses verbatim) and **KB5** (a second Librarian's fresh-eyes clause-level diff, in-scope sources) before returning `DONE` (`~/.kiro/skills/neo/references/shared/knowledge-base.md §7`, `~/.kiro/agents/librarian.md`).
