...
- JIRA IDs come from the TC's `**JIRA Ref:**` verbatim per [`~/.kiro/skills/neo/references/shared/jira-ref.md`](../shared/jira-ref.md) §2–3 (match exactly — same IDs, order, casing; never invent). Bracket-specific: when the TC has no JIRA Ref, OMIT the JIRA segment entirely — do NOT write an empty segment `[TC-001 -  - AC-001]` or a `—` inside the bracket.
...
4. **Assertion standards** — follow the Test Case Quality Rules from [`qa.md`](../roles/qa.md) (exact HTTP status codes, error body assertions, no duplicate scenarios)
