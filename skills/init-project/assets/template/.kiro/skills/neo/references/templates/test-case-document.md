...
- **Notes** (the `## Notes` section below) → a single **`<h2 id="notes">Notes</h2>` + `<ul>`** at the page end — the only home for cross-cutting test notes. **Apply [`html-output.md`](../html-output.md) §5.1 before any `<callout-box>`:** version/changelog (→ `VERSION.md`) and doc-vs-code-gap notes (→ `gap-analysis.md`) do NOT belong on the page (they fail `docverify.py`). `id="notes"` is load-bearing; omit when none.
...
These test cases are not executed in the Dev Loop. Re-evaluate once the upstream reference is resolved — see [`phase-map.md`](../phase-map.md) § Re-entry (AC Blocker resolved) for the re-entry workflow.

---

## JIRA Ref Inheritance Rule

Each TC's `JIRA Ref:` is **inherited verbatim from the AC at `Traces To`** — QA never picks or invents JIRA IDs; the AC document is the single source of truth. Full rules: [`~/.kiro/skills/neo/references/shared/jira-ref.md`](../shared/jira-ref.md) §2 — copy verbatim; OMIT the body line (and write `—` in the Summary column) when the AC has none; deduplicated union when a TC traces multiple ACs; re-sync when the AC changes upstream, never writing back an ID the AC lacks.
