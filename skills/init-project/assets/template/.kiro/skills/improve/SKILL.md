...
name: improve
description: >-
  Iteratively improve any output until a clear finish-line condition holds — autonomous
  loop modeled on the `/goal` pattern, ported as a Kiro Agent Skill. Establish ONE
  measurable completion condition up front (with adaptive questioning if vague), then
  run improve → self-evaluate → loop until the condition is met or the bound is reached.
  After each iteration the evaluator returns a yes/no + one-sentence reason; if "no",
  that reason becomes the next iteration's directive. Use to refine code, prose, data,
  config, design, or any artifact against a verifiable end state.
  Triggers on phrases like "improve this", "make it better", "iterate", "refine",
  "keep improving", "not good enough yet", "optimize this", "polish this",
  "tighten this up", "ปรับปรุง", "ทำให้ดีขึ้น", "ยังไม่ดี", "แก้ให้ดีกว่านี้",
  "iterate ต่อ", or when the user provides criteria and wants repeated improvement
  until they're satisfied. Also use when the user gives feedback on output and expects
  continued refinement, even without saying "improve" explicitly.
metadata:
  version: "4.0-kiro"
---
