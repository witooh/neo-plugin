...
name: gitlab
description: >
  Low-level GitLab `glab` execution/reference arm. The `neo` SKILL.md MR brain
  runs this skill's documented `glab` command set (via Bash) for MR **create** and
  review-comment **posting**; it is also usable directly for lightweight ops —
  read/summarize an MR, update an MR description, list MRs, view MR/CI status, fetch
  CI job logs, and approve MRs. NOTE: creating, reviewing, fixing, or addressing
  feedback on an MR now route through `neo` (which owns the thinking and the
  specialist dispatch) — not here. Trigger on a bare GitLab MR URL or "อ่าน MR",
  "ดู MR", "check MR", "สรุป MR", "อัพเดท MR", "update description", "list MRs",
  "check pipeline", "pipeline status", "approve MR", or a raw glab operation.
metadata:
  version: "1.1"
---
