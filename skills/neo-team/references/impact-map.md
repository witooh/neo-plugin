...
| 8a | **Review MR — no JIRA card**       | Code Reviewer ∥ Security ∥ QA (regression E2E)                                        | Read-only, run **in parallel** (orthogonal scopes) — see Propagation Rules § Parallel read-only review. QA runs the existing E2E suite to confirm no regression (no compliance check). No Developer dispatch unless the user explicitly asks to fix findings. Orchestrated by `neo-team` (SKILL.md § MR Workflows), which calls the `gitlab` skill for glab I/O. |
| 8b | **Review MR — with JIRA card**     | Code Reviewer ∥ Security ∥ QA (AC/TC compliance + regression)                         | As 8a, plus QA verifies the diff against the card's AC/TC by composing from the usecase's `acceptance-criteria.html` + `test-cases.html` + `traceability.html` (local docs only) and returns an AC/TC compliance table for the review comment. Card→usecase resolved via `docs/design/INDEX.md`; ambiguous → ask the user. |
...
| 11 | **Create MR**                      | invoke `Skill(gitlab)` MR Create                                                     | `neo-team` routes the request to the `gitlab` skill (execution arm), which analyzes the branch diff, generates the description, and runs `glab mr create`; neo-team reports the URL. Single delegation — no specialist dispatch, no plan UI. |
...
- **Parallel read-only review.** The read-only review group runs **concurrently**, not sequentially: **Code Reviewer ∥ Security** in row 4, and **Code Reviewer ∥ Security ∥ QA** in MR-review rows 8a/8b (QA in read-only MR-review mode — see `qa.md` § MR Review Mode). All are strictly read-only (CR1 / SEC1; QA in this mode does not write docs), their scopes are orthogonal (convention compliance / exploitability / AC-TC behavior), and none consumes another's output. The Orchestrator dispatches the whole group in a single batch (multiple `Agent` calls in one step) and shows **ONE combined checkpoint after all return** — the same exception to HARD-GATE 6 that the Dev loop uses. No worktree isolation is needed (read-only agents cannot conflict on files).
...
| "Review this diff" (raw code-only diff, review only)      | Code Reviewer        |
| "สร้าง MR" / "create MR"                                  | invoke `Skill(gitlab)` MR Create (delegation; reviewing an MR is rows 8a/8b — NOT single-role) |
