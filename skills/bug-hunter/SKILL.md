...
description: >-
  ...
  then BUG flow (do not fix in the hunt). Not for reported bugs the user already has a symptom
  for, or a diff review, gate audits (`falsifying`), or live HTTP abuse (`attack-test`).
---
...
- **Never fix in the hunt.** A confirmed finding is reported, not fixed. Do not edit production
  code in the hunt. The report **must** include how to reproduce and a proposed fix so
...
2. Build the check that must go red if it is real: a failing test, a request, a call with the
   boundary value. A hunt has no user-reported symptom to
...
- **Hand-off:** Do not fix in this hunt.
```
