...
neo is a thin engineering router plus a set of org-specific domain skills. The injected router owns the work: it decides loop vs graph, does the edits itself by default, delegates to specialist nodes when the work fans out or a step must fail in isolation, and stops only where a machine gate or a human confirm can prove the work. The generic method layer is vendored from the external `mattpocock/skills` plugin.
