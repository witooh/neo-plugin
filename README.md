...
| **`openapi-doc`** | Scan Go handler/router/usecase source → a single-file **OpenAPI 3.1** spec at `bruno/openapi.yaml` (info/servers/tags + inline paths + `components.schemas`), or update/validate against the code. Per-sentinel errors in `x-error-catalog`. Three-layer verify. Reads Go directly — Go is the source of truth. | "gen openapi", "สร้าง openapi spec", "openapi from go", "swagger spec from code", "อัปเดต openapi", "เช็ค openapi ตรงกับ code" |
...
│   ├── openapi-doc/         # Go → single-file OpenAPI 3.1 spec at bruno/openapi.yaml (source of truth)
