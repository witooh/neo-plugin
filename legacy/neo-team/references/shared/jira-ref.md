...
- **Capture trigger.** When the prompt names JIRA card IDs (format `ABC-123` — all-caps project key + dash + integer), capture each ID exactly as written and attach it to the AC(s) the user associates with it. Examples:
  ...
  - "AC-002 มาจาก PROJ-456 และ PROJ-789" → AC-002 gets both; other ACs do NOT inherit
...
- **Mapping ambiguity.** If IDs are named but their AC mapping is unclear, raise an Open Question (Universal Rule) asking which AC(s) each ID covers. Do NOT default to "all ACs get all IDs" — that destroys traceability granularity.
...
- **Never invent** a JIRA ID the user did not provide (Universal Rule — Never Guess). If the user said "the JIRA card for login" with no ID, ask which ID — do not write a placeholder.
