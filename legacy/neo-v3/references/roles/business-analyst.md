...
- **Fill the Build Plan — whenever `docs/design/<usecase>/traceability.html` exists on disk** (NOT only when Design ran this session; a re-run on an already-analysed card has it): read its per-AC design-element mapping (`#ac-design` `File / Method`) + Pending Items Surface (`#pending`) → fill both Build Plan tiers (`../shared/task-tracking.md` §5–§6): **`### Ready to build now`** (a checklist line per Ready, unbuilt unit, named to `layer · file/method · serves AC-IDs`; no big-AC-only gate) + **`### Blocked on upstream`** (the table of work gated by pending upstream fields). Also **seed the `Blocked on upstream` tier from the shared `Blocker:` refs** you already wrote — two or more ACs sharing a blocker are one shared item, derivable even before traceability exists. No Architect change needed.
